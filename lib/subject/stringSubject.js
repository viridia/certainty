...
StringSubject.prototype.isEmpty = function () {
  if (this.value !== '') {
    this.failureStrategy.fail('Expected ' + this.describe() + ' to be the empty string.');
  }
  return this;
};
...
StringSubject.prototype.includes = function (expected) {
  if (!this.value.includes(expected)) {
    this.failureStrategy.fail('Expected ' + this.describe() + ' to include ' +
      format(expected) + '.');
  }
  return this;
};
...
StringSubject.prototype.startsWith = function (expected) {
  if (!this.value.startsWith(expected)) {
    this.failureStrategy.fail('Expected ' + this.describe() + ' to start with ' +
      format(expected) + '.');
  }
  return this;
};
...
StringSubject.prototype.endsWith = function (expected) {
  if (!this.value.endsWith(expected)) {
    this.failureStrategy.fail('Expected ' + this.describe() + ' to end with ' +
      format(expected) + '.');
  }
  return this;
};
