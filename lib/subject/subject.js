...
Subject.prototype.isDeeplyEqualTo = function (expected) {
  var d = diff(expected, this.value, this.name, true, 4);
  if (d.length > 0) {
    this.fail(d.join('\n'));
  }
  return this;
...
/** Ensure that the subject value is approximately equal to the expected value.
    @param {*} expected The expected value.
    @return {Subject} `this` for chaining.
*/
Subject.prototype.isApproximately = function (expected, opt_epsilon) {
  var epsilon = opt_epsilon || .00000001;
  if (Math.abs(this.value - expected) > epsilon) {
    this.failComparison('approximately equal to', this.format(expected));
  }
  return this;
};
